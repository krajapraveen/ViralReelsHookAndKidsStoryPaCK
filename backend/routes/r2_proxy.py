...
async def r2_proxy(path: str, request: Request):
    ...
    if cached_url:
        ...
        # P0 2026-06 SECURITY — `cached_url` is the previously-generated
        # boto3 presigned URL for THIS path. Host is fixed to our R2
        # endpoint by boto3 config (`<account>.r2.cloudflarestorage.com`).
        # The user-controlled `path` becomes the S3 Key, not the host —
        # the `..` guard above blocks traversal abuse. Audited 2026-06.
    ...
    try:
        ...
        from fastapi.responses import RedirectResponse
        # P0 2026-06 SECURITY — `presigned_url` is generated by boto3
        # using `endpoint_url=https://<account>.r2.cloudflarestorage.com`
        # (a server-controlled env var). The user-controlled `path` is
        # the S3 Key, NOT the host. Path traversal blocked above.
