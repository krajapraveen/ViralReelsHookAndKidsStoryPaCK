...
async def handle_renewal_webhook(request: Request):
    ...
    try:
        ...
        # In production, verify signature using CASHFREE_WEBHOOK_SECRET
        ...
        if event_type == "SUBSCRIPTION_PAYMENT_SUCCESS":
            ...
            if subscription_id:
                await process_renewal(subscription_id)
            ...
            if subscription_id:
                await db.subscriptions.update_one(
                    {"id": subscription_id},
                    {"$set": {"paymentStatus": "FAILED", "updatedAt": datetime.now(timezone.utc).isoformat()}}
                )
