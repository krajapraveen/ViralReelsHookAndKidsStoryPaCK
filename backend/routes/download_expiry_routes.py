...
async def get_my_downloads(user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Failed to get temp downloads: {e}")
            ...
            for job in comic_jobs:
                ...
                if job.get("resultUrl"):
                    ...
                    downloads.append({
                        ...
                        "expires_at": None,
                        "downloaded": False,
                        "preview_url": result_url if result_url.startswith("data:") else None,
                    ...
                    for i, panel in enumerate(job.get("panels", [])):
                        ...
                        if image_url:
                            downloads.append({
                                ...
                                "expires_at": None,
                                "downloaded": False,
                                "preview_url": image_url if image_url.startswith("data:") else None,
                                "download_url": image_url
                            })
        except Exception as e:
            logger.warning(f"Failed to get comic jobs: {e}")
