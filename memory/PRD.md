...
3. **Protocol**: Content-Type from R2 metadata (authoritative). nosniff safe. ETag, Content-Disposition: inline, CORS.
4. **Streaming**: Videos streamed 64KB chunks. Range <2MB buffered for exact Content-Length. Surrogate-Control bypasses ingress.
5. **Frontend Priority**: Hero eager+opacity0.6. First 2 rows eager (no IO gate). First 6 cards loading="eager".

### Platform Constraints (K8s Ingress)
- Strips Content-Length from GET (HEAD preserves it)
- Strips Accept-Ranges from GET (HEAD preserves it)
