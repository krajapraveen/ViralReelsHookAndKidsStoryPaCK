...
export default function CharacterConsistencyStudio() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-950 via-purple-950 to-slate-950">
      {/* Header */}
      <header className="sticky top-0 z-50 bg-slate-950/80 backdrop-blur-lg border-b border-white/10 vs-page-header">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2.5 sm:py-4 flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-4 min-w-0 vs-header-left">
            <Link to="/app/story-video-studio">
              <Button variant="ghost" size="icon" className="text-white flex-shrink-0 h-9 w-9">
                <ArrowLeft className="w-5 h-5" />
              </Button>
            </Link>
            <div className="min-w-0">
              <h1 className="text-base sm:text-xl font-bold text-white flex items-center gap-2 vs-header-title">
                <Users className="w-5 h-5 sm:w-6 sm:h-6 text-purple-400 flex-shrink-0" />
                <span className="truncate">Character Studio</span>
              </h1>
              <p className="text-xs sm:text-sm text-slate-400 vs-header-subtitle">Create and manage consistent characters</p>
            </div>
          </div>

          <Button
            onClick={() => setShowCreateModal(true)}
            className="bg-purple-500 hover:bg-purple-600 flex-shrink-0 text-xs sm:text-sm"
            size="sm"
