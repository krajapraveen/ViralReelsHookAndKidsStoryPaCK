...
import { GitBranch, ChevronRight, Play, X, TrendingUp } from 'lucide-react';
...
export function ActiveChainsChip() {
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={() => setOpen(!open)} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 transition-colors" data-testid="active-chains-chip">
        <GitBranch className="w-3.5 h-3.5" />
        <span className="hidden sm:inline">Stories</span>
        {incomplete > 0 && <span className="min-w-[16px] h-[16px] bg-amber-500 rounded-full flex items-center justify-center text-[10px] font-bold text-black" data-testid="chains-badge">{incomplete}</span>}
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 rounded-xl border border-slate-700 bg-slate-900 shadow-2xl shadow-black/40 z-50 overflow-hidden" data-testid="resume-drawer">
              ...
              <div key={c.chain_id} className="p-3 hover:bg-slate-800/50 transition-colors" data-testid={`drawer-chain-${c.chain_id}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {c.momentum_msg && <p className="text-[10px] text-amber-400/70 mt-0.5 flex items-center gap-0.5"><TrendingUp className="w-2.5 h-2.5" /> {c.momentum_msg}</p>}
                  </div>
                  <div className="shrink-0">
                    {c.continue_job_id ? (
                      <Button size="sm" onClick={() => { setOpen(false); navigate(`/app/photo-to-comic?continue=${c.continue_job_id}`); }} className="bg-indigo-600 hover:bg-indigo-700 h-7 px-2.5 text-[10px]" data-testid={`drawer-continue-${c.chain_id}`}>
                        <Play className="w-3 h-3 mr-1" /> Continue
                      </Button>
                    ) : (
                      <Button size="sm" variant="outline" onClick={() => { setOpen(false); navigate(`/app/story-chain/${c.chain_id}`); }} className="border-slate-700 text-slate-300 h-7 px-2.5 text-[10px]" data-testid={`drawer-view-${c.chain_id}`}>
          ...
          <div className="px-4 py-2.5 border-t border-slate-800 bg-slate-900/50">
            <button onClick={() => { setOpen(false); navigate('/app/my-stories'); }} className="text-xs text-indigo-400 hover:text-indigo-300 flex items-center gap-1 transition-colors" data-testid="drawer-all-stories">
