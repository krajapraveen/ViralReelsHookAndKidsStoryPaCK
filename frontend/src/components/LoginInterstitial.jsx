...
import { BookOpen, Sparkles, X, Play, GitBranch } from 'lucide-react';
...
export function LoginInterstitial() {
  ...
  useEffect(() => {
    if (sessionStorage.getItem('interstitial_dismissed')) return;
    (async () => {
      try {
      ...
      <div className="w-full max-w-md mx-4 rounded-2xl border border-indigo-500/30 bg-slate-900 overflow-hidden shadow-2xl shadow-indigo-500/10 animate-in fade-in zoom-in-95 duration-300">
        <div className="relative h-40 overflow-hidden">
          <SafeImage
            ...
            alt="Story preview"
            aspectRatio="auto"
            titleOverlay={chain.root_style || 'Your Story'}
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            {chain.momentum_msg && <p className="text-xs text-amber-400/80">{chain.momentum_msg}</p>}
          </div>
          ...
          <div className="flex gap-2 pt-1">
            <Button onClick={handleContinue} className="flex-1 bg-indigo-600 hover:bg-indigo-700 h-11" data-testid="interstitial-continue-btn">
              {chain.continue_job_id ? <><Play className="w-4 h-4 mr-2" /> Next Episode</> : <><Sparkles className="w-4 h-4 mr-2" /> View Chain</>}
            </Button>
            <Button variant="ghost" onClick={dismiss} className="text-slate-400 hover:text-white h-11 px-4" data-testid="interstitial-skip-btn">Later</Button>
          </div>
        </div>
      </div>
    </div>
